package com.payflow.merchant.api.security;
...
public class ApiKeyAuthenticationFilter extends OncePerRequestFilter {
    ...
    public ApiKeyAuthenticationFilter(MerchantApplicationService merchantApplicationService, ObjectMapper objectMapper) {
    ...
    protected void doFilterInternal(
        ...
        if (isPublicMerchantRegistration(request.getMethod(), path)) {
            filterChain.doFilter(request, response);
            return;
        }
        ...
        try {
            Merchant merchant = merchantApplicationService.authenticate(token);
            ...
            writeUnauthorized(response, request, "invalid_api_key", e.getMessage());
        }
    }

    private static boolean isPublicMerchantRegistration(String method, String path) {
        if (!"POST".equalsIgnoreCase(method)) {
            return false;
        }
        return "/v1/merchants".equals(path) || "/v1/merchants/".equals(path);
    }
