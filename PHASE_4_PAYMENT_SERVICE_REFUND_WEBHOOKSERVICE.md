...
## Local testing with the built-in webhook receiver

For local development and testing, PayFlow includes a **webhook-receiver** — a simple Node.js server that receives webhooks and provides a REST API to inspect them.

### Starting the webhook receiver

The webhook-receiver is automatically included in `infra/docker-compose.yml`. It starts alongside the other services:
...
The receiver runs on port **9000** and is accessible at:
- **UI/API:** http://localhost:9000/webhooks
- **Health check:** http://localhost:9000/health

### Registering a webhook for local testing

In the frontend (http://localhost:3000/webhooks), register a webhook using the Docker internal URL:

```
http://webhook-receiver:9000/webhook
```

Select the event types you want to receive (e.g., `payment.captured`, `payment.created`).

### Viewing received webhooks

Once a payment event occurs (e.g., you create and capture a payment), you can view the received webhooks in several ways:

1. **Docker logs** — the receiver prints every webhook to stdout:
   ```bash
   docker logs -f payflow-webhook-receiver-1
   ```

2. **API endpoint** — view all received webhooks:
   ```bash
   curl http://localhost:9000/webhooks
   ```

3. **Browser** — open in your browser:
   - http://localhost:9000/webhooks — JSON list of all webhooks
   - http://localhost:9000/webhook/latest — most recent webhook

4. **Clear received webhooks:**
   ```bash
   curl -X DELETE http://localhost:9000/webhooks
   ```
...
```json
{
  ...
  "aggregateId": "pay_xxx",
  "merchantId": "mer_xxx",
  "occurredAt": "2026-05-19T21:42:50.912925Z",
  "payload": {
    "amount": 100,
    "currency": "USD",
    "paymentId": "pay_xxx",
    "capturedAt": "2026-05-19T21:42:50.912574607Z",
    "merchantId": "mer_xxx"
  }
}
```

The webhook is signed with HMAC-SHA256. The signature is sent in the `Payflow-Signature` header (hex-encoded). You can verify it using the secret returned when you registered the webhook.

### Configuration for local development

The webhook-service is configured to allow HTTP URLs for local testing. This is enabled via:

```yaml
# docker-compose.yml - webhook-service environment
PAYFLOW_WEBHOOK_ALLOW_HTTP: "true"
```

This allows registering webhooks with `http://` URLs instead of requiring `https://`.

### Troubleshooting

- **Webhook not received?** Check the delivery status in the database:
  ```bash
  docker exec payflow-postgres-1 psql -U payflow -c \
    "SELECT id, event_type, status, attempts, last_error FROM webhooks.webhook_deliveries ORDER BY created_at DESC LIMIT 5;"
  ```

- **Connection refused?** Verify both containers are on the same Docker network:
  ```bash
  docker network inspect payflow_default
  ```
...
- Webhook receiver (testing): `webhook-receiver/`
